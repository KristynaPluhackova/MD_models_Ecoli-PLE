...
B="$SDIR/backward.py -f $INP -raw $RAW -o $GRO -kick $KICK -sol -p $TOP -po $OTP -n $NDX -from $CG -to $AA $NOPBC"
echo $B; $B || exit
...
# Run
M="gmx mdrun -deffnm $BASE -v -nt $EMNP"
echo $M; $M 
...
#    $SDIR/qfit.py $AAS $GRO > posref.gro    ### commented out by Kri because it didn't work
    PR="-r posref.gro"
else
    PR="-r $BW"
fi
...
__MDP__
  ...
  # Set up run
  G="gmx grompp -f $mdp -c $GRO -p $OTP -o $BASE -maxwarn 2 $PR"
  echo $G; $G || exit
...
do
  ...
  m=${mdp%.mdp}
  tag=$((++i))-md-${m##*/}
  G="gmx grompp -f $mdp -c $GRO -p $OTP -o $tag -maxwarn 2 $PR"
  echo $G; $G || exit
  M="gmx mdrun -deffnm $tag -v -nt $NP"
  echo $M; $M 
  GRO=$tag.gro
...
# Copy one tpr file to keep it
cp 1-EM.tpr topol.tpr
